...
export default function Sidebar({ phase, activeTab = "dashboard", setActiveTab }) {
  const userName = localStorage.getItem("userName") || "Janhavi";
  const userEmail = localStorage.getItem("userEmail") || "janhavikhare09@gmail.com";
      ...
      <div>
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-2xl bg-gradient-to-tr from-pink-500 to-purple-600 flex items-center justify-center text-white shadow-[0_0_20px_rgba(236,72,153,0.4)]">
          ...
          <button className="relative w-8 h-8 rounded-full bg-white/[0.05] border border-white/[0.08] flex items-center justify-center text-slate-400 hover:text-white transition">
          ...
          {navItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`relative flex items-center gap-3.5 w-full px-4 py-3 rounded-2xl font-semibold text-sm transition-all ${
                  isActive
                    ? "bg-white/[0.08] text-white shadow-[0_0_15px_rgba(236,72,153,0.15)]"
                    : "text-slate-400 hover:text-white hover:bg-white/[0.04]"
                }`}
              >
                {isActive && (
                  <span className="absolute left-0 top-2.5 bottom-2.5 w-1 rounded-r-full bg-gradient-to-b from-pink-500 to-purple-500 shadow-[0_0_8px_rgba(236,72,153,0.8)]"></span>
                )}
                <Icon className={isActive ? "text-pink-400 text-base" : "text-base"} />
